...
export function FullPlayer() {
  ...
  useEffect(() => {
    ...
    let cancelled = false
    void resolveArtworkSrc(currentSong.artwork, currentSong.id).then((url) => {
      if (!cancelled) setBlurArtwork(url)
    })
    ...
    >
      ...
      <div style={{ position: 'relative', zIndex: 1, flex: 1, display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', padding: '16px 20px', gap: 12 }}>
          ...
          <IconButton onClick={() => setShowQueue(!showQueue)} active={showQueue}>
        ...
        {showQueue ? (
          ...
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '20px 40px', gap: 24 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 24, width: '100%', justifyContent: 'center' }}>
              ...
              <button
                ...
                style={{
                  ...
                  boxShadow: '0 8px 24px rgba(255,32,32,0.4)',
              ...
              >
                {repeatMode === 'one'
                  ? <svg width="22" height="22" viewBox="0 0 24 24" fill="currentColor"><path d="M7 7h10v3l4-4-4-4v3H5v6h2V7zm10 10H7v-3l-4 4 4 4v-3h12v-6h-2v4zm-4-2V9h-1l-2 1v1h1.5v4H13z"/></svg>
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', justifyContent: 'center' }}>
              {currentSong.format && (
                <span style={{ fontSize: 11, padding: '3px 8px', borderRadius: 20, background: 'rgba(255,32,32,0.15)', color: 'var(--accent)', fontWeight: 600, textTransform: 'uppercase' }}>
                  {currentSong.format}
                </span>
              )}
              {currentSong.isLossless && (
                <span style={{ fontSize: 11, padding: '3px 8px', borderRadius: 20, background: 'rgba(255,32,32,0.08)', color: 'var(--accent)', border: '1px solid var(--border-accent)' }}>
                  LOSSLESS
                </span>
              )}
            </div>
