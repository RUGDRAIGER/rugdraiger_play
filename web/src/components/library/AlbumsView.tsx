  ...
  if (selected) {
    ...
    return (
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        <div style={{ padding: '20px 28px 16px', flexShrink: 0, display: 'flex', alignItems: 'center', gap: 16 }}>
          <button onClick={() => setSelected(null)} style={{ color: 'var(--text-secondary)', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4, fontSize: 14 }}>
            <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M20 11H7.83l5.59-5.59L12 4l-8 8 8 8 1.41-1.41L7.83 13H20v-2z"/></svg>
            Álbumes
          </button>
        </div>
        <div style={{ display: 'flex', gap: 24, padding: '0 28px 24px', alignItems: 'flex-start', flexShrink: 0, flexWrap: 'wrap' }}>
          <AlbumCoverArt
          ...
          />
          <div>
            <div style={{ fontSize: 24, fontWeight: 800, marginBottom: 4 }}>{selected.title}</div>
            ...
            <button
              ...
              style={{ padding: '10px 20px', borderRadius: 'var(--radius-md)', background: 'var(--accent)', color: '#fff', fontSize: 14, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}
            >
        ...
        <div className="scrollable" style={{ flex: 1, padding: '0 28px' }} onClick={() => setMenuState(null)}>
          {albumSongs.map((song, i) => (
            ...
            >
              <span style={{ width: 28, textAlign: 'center', fontSize: 12, color: 'var(--text-tertiary)', flexShrink: 0 }}>{song.trackNumber || i + 1}</span>
