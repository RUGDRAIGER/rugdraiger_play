...
const sections = [
  { id: 'home' as ViewName, label: 'Inicio', desc: 'Dashboard', icon: <svg width="28" height="28" viewBox="0 0 24 24" fill="currentColor"><path d="M10 20v-6h4v6h5v-8h3L12 3 2 12h3v8z"/></svg> },
  ...
  { id: 'genres' as ViewName, label: 'Géneros', desc: 'Por género', icon: <svg width="28" height="28" viewBox="0 0 24 24" fill="currentColor"><path d="M12 3v10.55c-.59-.34-1.27-.55-2-.55-2.21 0-4 1.79-4 4s1.79 4 4 4 4-1.79 4-4V7h4V3h-6z"/></svg> },
...
export function LibraryView({ onNavigate }: Props) {
  const { songs, albums, artists, genres } = useLibraryStore()
  const counts: Record<string, number> = { home: 0, songs: songs.length, albums: albums.length, artists: artists.length, genres: genres.length, playlists: 0 }
